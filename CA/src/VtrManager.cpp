...
VtrManager
--------------------------------------------------------------------*/
VtrManager::VtrManager(CellularAutomataManager_3D *caManager)
  :VtkManager(), 
...
--------------------------------------------------------------------*/
void
VtrManager::load(const YAML::Node& node)
{
  // Make some things pretty in the log file
  CafeEnv::self().caOutputP0() << "\n" << "VtrManager Review" << "\n";
  CafeEnv::self().caOutputP0() << "=============================" << "\n";

  YAML::Node vtr_output = node["output"];
  if (vtr_output)
  {
    ...
    // output variables
    YAML::Node varList = vtr_output["output_variables"];
    if (varList)
    {
...
----------------------------------------------------------------------*/
void
VtrManager::vtr_write_cell_data()
{
      ...
      for (int k = 1; k<nzLocGhost_ - 1; k++)
      {
        for (int j = 1; j < nyLocGhost_ - 1; j++)
        {
          ...
          for (int i = 1; i < nxLocGhost_ - 1; i++)
          {
            ...
            if(!caManager_->shafferVtrID_ && val >= 0)
              val = val;
            else if (val >= 0 && caManager_->mappedGrainID!=NULL)
...
----------------------------------------------------------------------*/
void
VtrManager::vtr_write_coordinates()
{
...
VtrManager::pvtr_generator()
{
  ...
  for (int iVar = 0; iVar < numOutputVar; iVar++)
  {
    ...
    case 11: //debug
    {
        pvtrOut_ << CAFE::FourthEleWidth_ << "<PDataArray type=\"Int8\" Name=\"growth_flag\"  format=\"ascii\"/>\n";
        pvtrOut_ << CAFE::FourthEleWidth_ << "<PDataArray type=\"Float64\" Name=\"cooling\"  format=\"ascii\"/>\n";
        break;
    }
    case 12: //debugMC
    {
        pvtrOut_ << CAFE::FourthEleWidth_ << "<PDataArray type=\"Float64\" Name=\"p\"  format=\"ascii\"/>\n";
        pvtrOut_ << CAFE::FourthEleWidth_ << "<PDataArray type=\"Float64\" Name=\"tMCS\"  format=\"ascii\"/>\n";
        pvtrOut_ << CAFE::FourthEleWidth_ << "<PDataArray type=\"Float64\" Name=\"vf_a\"  format=\"ascii\"/>\n";
        pvtrOut_ << CAFE::FourthEleWidth_ << "<PDataArray type=\"Float64\" Name=\"vf_b\"  format=\"ascii\"/>\n";
        pvtrOut_ << CAFE::FourthEleWidth_ << "<PDataArray type=\"Float64\" Name=\"Hardness\"  format=\"ascii\"/>\n";
        break;
    }
    }
  }
...
bool
VtrManager::execute(int numStep, double time)
{
